...
public class ObservationTdbRepository implements ObservationRepository {
    ...
    public Set<Observation> getAllObservationsFromSensor(int id) {
        Dataset dataset = datasetProvider.guardedDataset();
        ...
        while (iterator.hasNext()){
            observation = iterator.next();
            int sensorId = Integer.parseInt(observation.getTdbSensorId().substring(25));
            if(sensorId == id)
                observationsList.add(observation);
        }

        Set<Observation> observationSet = new HashSet<>();
        observationSet.addAll(observationsList);
